...
# Planner-guided candidate-region settings
USE_PLANNER_CANDIDATE_REGIONS = True
MAX_CANDIDATE_REGIONS = 5
MIN_REGION_WIDTH_PX = 160
MIN_REGION_HEIGHT_PX = 160
...
def recover_planner_regions_from_text(text: str) -> dict[str, Any]:
    """
    Last-resort recovery for planner JSON when Gemini returns extra text,
    markdown fences, or a partially malformed response.

    This only accepts complete candidate objects that contain the required
    score and coordinate fields.
    """
    ...
    for attempt in range(1, ICON_DETECTION_RETRIES + 1):
        try:
            if _icon_cache["x"] is not None:
                ...
                if confidence < VERIFICATION_SKIP_CONFIDENCE:
                    if not verify_icon_identity(pil_image, x_shot / pil_image.width, y_shot / pil_image.height):
